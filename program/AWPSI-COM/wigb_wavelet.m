function wigb_wavelet(nstart,nend,qwavelet,scal,x,z,amx)
...
%  
 ...
 nx=length(nstart);
%  [~,nx]=size(a);
 nz=nend(end)-nstart(1)+1;
 A=zeros(nz,nx);
 for i=1:nx
     A(nstart(i):nend(i),i)=-qwavelet(:,i);
 end
 a=A;
...
%12/7/97 'Xdata', x(i)+[0 0], 'Ydata',[z0 z1]*dz);	% remove zero line
	...
	line( 'Color',linecolor,'EraseMode','background',  ...
	 'LineWidth',linewidth, ...
	 'Ydata', tr(nstart(i):nend(i))+x(i), 'Xdata',z(nstart(i):nend(i)));	% negatives line
ww=1;
   else % zeros trace
line( 'Color',linecolor,'EraseMode','background',  ...
	 'LineWidth',linewidth, ...
