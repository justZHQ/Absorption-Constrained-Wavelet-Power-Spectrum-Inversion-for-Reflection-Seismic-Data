function [log_ampx_Correction,ampx_Correction]=Fast_COM(mu,lambda,ampx,df,p,Iterations,Begf,Endf,Begf_high,Endf_low,GausiannVariance)
...
Para=exp(-T.^2/(2*GausiannVariance^2))*lambda;
% figure;
% plot(Para);
Lam2=[];
for i=1:n
...
%         Pi(j,j)=-1;
    ...
    Gii=L1*Pi*A_allcut;
        Gi=L2*Pi*A_allcut;
%     Gall=Gall+(Gii'*Gii);
        Gall=Gall+mu*(Gi'*Gi)+lambda*(Gii'*Gii);
end
